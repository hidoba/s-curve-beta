...
def plot_blend_comparison():
    """
    Show the Curve Blend approach for mid-motion redirection:
    ...
    # Curve Blend approach only
    plan2 = continue_motion_blend(plan1, x_target=x_new_target,
                                   robotVmax=robotVmax, robotAmax=robotAmax,
                                   t_interrupt=t_interrupt, mode='blend')
    T2 = plan2['T']
    motion2 = plan2['blend_motion']

    print(f"\nCurve Blend: x_orig×(1-β) + x_new×β:")
    print(f"  Duration: T={T2:.3f}s")
    print(f"  T_remaining_orig={motion2.T_remaining_orig:.3f}s")
    print(f"  T_redirect={motion2.T_redirect:.3f}s (time for {motion2.x_orig_end:.1f} → {x_new_target})")
    ...
    plt.suptitle('Mid-Motion Redirection with C∞ Continuity\n'
                 'Curve Blend: smooth transition to new target',
